...
namespace PhoneStation
{
    public partial class InfoStaff : Form
    {
        ...
        private void button_Save_Click(object sender, EventArgs e)
        {
            ...
            DateTime startWorking = dateOfStartWorking.Value;

            int passp = -1;
            try
            {
                passp = Int32.Parse(passport.Text);
            }
            catch (Exception)
            {
            ...
            OleDbCommand command = new OleDbCommand("changeStaffInfo", cn);
            command.CommandType = CommandType.StoredProcedure;

            command.Parameters.AddWithValue("@idStaff", id);
            command.Parameters.AddWithValue("@name", name);
            command.Parameters.AddWithValue("@birthday", birth);
            command.Parameters.AddWithValue("@passport", passp);
            command.Parameters.AddWithValue("@dateOfStartWorking", startWorking);
